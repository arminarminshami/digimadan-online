// ------------------------------------------------------------
// تبدیل تاریخ میلادی <-> شمسی (بدون نیاز به کتابخانه‌ی خارجی)
// الگوریتم استاندارد و شناخته‌شده‌ی تقویم جلالی (jalaali).
// تاریخ همچنان در دیتابیس به‌صورت میلادی (YYYY-MM-DD) ذخیره می‌شود؛
// این فایل فقط برای نمایش/دریافت شمسی در فرم استفاده می‌شود.
// ------------------------------------------------------------
...
// 'YYYY-MM-DD' میلادی -> { jy, jm, jd } یا null
export function isoToJalali(isoDate) {
  if (!isoDate) return null
  // هم 'YYYY-MM-DD' و هم تاریخ کامل ISO (با ساعت) پشتیبانی می‌شود
...
// { jy, jm, jd } -> 'YYYY-MM-DD' میلادی (برای ذخیره در دیتابیس)
