...
export async function getAuctionDocumentUrl(auctionId, docUrl) {
  // invokeFn پیام واقعی سرور را نشان می‌دهد (مثلا «مهلت دریافت مدارک
  // به پایان رسیده است») به‌جای یک پیام کلی و بی‌فایده.
  const data = await invokeFn('auction-doc-access', { auctionId, docUrl }, 'دریافت فایل با خطا مواجه شد.')
  return data.url
}
