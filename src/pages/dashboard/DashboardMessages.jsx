...
export default function DashboardMessages({ user }) {
  ...
  async function handleSend(e) {
    ...
    try {
      ...
      // اگر ارسال نشد، متن به کادر برگردانده می‌شود؛ قبلا پیام کاربر
      // بی‌صدا از بین می‌رفت و او فکر می‌کرد ارسال شده است.
      setText(content)
      setSendError('ارسال پیام انجام نشد. لطفاً دوباره تلاش کنید.')
    } finally {
      setSending(false)
    }
  }
  ...
  return (
    <div className="dashboard-messages">
      ...
      <ErrorModal message={sendError} onClose={() => setSendError(null)} />
    </div>
  )
}
